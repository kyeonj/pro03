    ...
    <div class="container">
      <h2 class="title">이미지 추가하기</h2>
		<form name="frm1" id="frm1" action="${path1 }/ModifyProTourCtrl.do" method="post" onsubmit="return tourCheck(this)">
			      ...
			      <td>
					...
					<div class="field">
						  ...
						  <select name="cate" id="cate" class="select" onchange="changeTourNo()" required>
						    ...
						    <option value="A">명소</option>
						    <option value="B">음식</option>
						    <option value="C">쇼핑</option>
			...
			<div class="buttons">
			  <a href="${path1 }/GetTourListCtrl.do" class="button is-link is-light">목록</a>
			  <c:if test='${sid.equals("admin") }'>
				  <a href="${path1 }/GetTourDetailCtrl.do?no=${dto.no }" class="button is-link is-light">장소 보기</a>
				  <button type="submit" class="button is-link is-light">장소 수정</button>
			  </c:if>
			</div>
		</form>
    </div>
